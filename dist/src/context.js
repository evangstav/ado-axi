...
function resolveOrgProjectRepo(flagValue) {
    if (flagValue) {
...
function parseTriple(value, source) {
...
export function parseRemoteUrl(url) {
    ...
    const visualStudio = url.match(/([^/.@]+)\.visualstudio\.com\/([^/]+)\/_git\/([^/]+?)(?:\.git)?$/);
    if (visualStudio) {
        return {
            org: visualStudio[1],
